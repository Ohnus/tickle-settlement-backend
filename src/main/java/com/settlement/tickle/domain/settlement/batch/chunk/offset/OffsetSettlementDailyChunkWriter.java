...
// 청크별 합산을 Map에 담고 UPSERT하여 최종 집계한다.
@RequiredArgsConstructor
public class OffsetSettlementDailyChunkWriter implements ItemWriter<OffsetSettlementEntryRow> {
    ...
    @Override
    public void write(Chunk<? extends OffsetSettlementEntryRow> chunk) {
        Map<GroupKey, Totals> grouped = new LinkedHashMap<>();
        for (OffsetSettlementEntryRow row : chunk) {
    ...
    private record GroupKey(Long memberId, Long performanceId, String performanceTitle) {
        static GroupKey of(OffsetSettlementEntryRow row) {
    ...
    private static class Totals {
        ...
        void add(OffsetSettlementEntryRow row) {
