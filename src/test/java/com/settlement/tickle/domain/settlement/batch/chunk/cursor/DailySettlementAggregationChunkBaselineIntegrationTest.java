...
@ActiveProfiles("test")
@Testcontainers
class DailySettlementAggregationChunkBaselineIntegrationTest {
    ...
    @Autowired
    @Qualifier("dailySettlementAggregationChunkBaselineJob")
    private Job chunkBaselineJob;
    ...
    @BeforeEach
    void setUp() {
        jobLauncherTestUtils.setJob(chunkBaselineJob);

        host = memberRepository.save(Member.builder()
                .email("chunk-host@t.io").password("encoded").nickname("chunkHost").role(MemberRoleType.HOST)
                .build());
        buyer = memberRepository.save(Member.builder()
                .email("chunk-buyer@t.io").password("encoded").nickname("chunkBuyer").role(MemberRoleType.MEMBER)
                .build());
        ...
        performance = performanceRepository.save(Performance.builder()
                .member(host).status(performanceStatus).title("청크 baseline 테스트 공연").price(10000)
    ...
    private void createSettlementEntry(long salesAmount, String settlementStatusDescription, LocalDateTime createdAt) {
        ...
        Reservation reservation = reservationRepository.save(Reservation.builder()
                ...
                .code("CHUNK-" + System.nanoTime()).price((int) salesAmount)
                .build());
    ...
    @Test
    @DisplayName("Tasklet 버전과 같은 시나리오(WAITING 합산/CANCELED 제외/다른 날짜 제외)에서 같은 결과를 낸다.")
    void aggregatesSameAsTaskletVersion() throws Exception {

        // given
        LocalDate targetDate = LocalDate.of(2031, 3, 20);
