...
@ActiveProfiles("test")
@Testcontainers
class DailySettlementAggregationChunkOffsetIntegrationTest {
    ...
    @Autowired
    @Qualifier("dailySettlementAggregationChunkOffsetJob")
    private Job chunkOffsetJob;
    ...
    @BeforeEach
    void setUp() {
        jobLauncherTestUtils.setJob(chunkOffsetJob);

        host = memberRepository.save(Member.builder()
                .email("offset-host@t.io").password("encoded").nickname("offsetHost").role(MemberRoleType.HOST)
                .build());
        buyer = memberRepository.save(Member.builder()
                .email("offset-buyer@t.io").password("encoded").nickname("ofstBuyer").role(MemberRoleType.MEMBER)
                .build());
        ...
        performance = performanceRepository.save(Performance.builder()
                .member(host).status(performanceStatus).title("OFFSET baseline 테스트 공연").price(10000)
    ...
    private void createSettlementEntry(long salesAmount, String settlementStatusDescription, LocalDateTime createdAt) {
        ...
        Reservation reservation = reservationRepository.save(Reservation.builder()
                ...
                .code("OFFSET-" + System.nanoTime()).price((int) salesAmount)
                .build());
    ...
    @Test
    @DisplayName("Tasklet/커서/키셋 버전과 같은 시나리오에서 같은 결과를 낸다.")
    void aggregatesSameAsOtherVersions() throws Exception {

        // given
        LocalDate targetDate = LocalDate.of(2031, 4, 10);
